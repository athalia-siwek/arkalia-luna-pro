...
class CrossModuleValidator:
    ...
    async def run_validation_loop(self):
        ...
        while self.is_running:
            try:
                validation = await self.validate_states()
                logger.info(f"✅ Validation: score={validation.coherence_score:.2f}, conflits={len(validation.conflicts)}")
...
async def get_status():
    ...
    return {
        ...
        "last_validation": validator.last_validation.dict() if validator.last_validation else None
    }
