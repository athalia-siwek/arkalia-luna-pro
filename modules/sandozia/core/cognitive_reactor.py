...
class CognitiveReactor:
    ...
    async def check_and_react(self, context: Dict, decision_pattern_count: int = 0) -> List[CognitiveReaction]:
        """
    ...
    async def _trigger_cognitive_pause(self, repetition_count: int) -> Optional[CognitiveReaction]:
    ...
    async def _check_berserk_mode(self, context: Dict) -> Optional[CognitiveReaction]:
        ...
        if global_score < self.config["berserk_threshold"]:
            ...
            try:
                ...
                emergency_file.write_text(f"berserk_mode:{datetime.now().isoformat()}:score_{global_score}")
    ...
    async def quarantine_module(self, module_name: str, reason: QuarantineReason, 
                               duration_minutes: Optional[int] = None, metadata: Optional[Dict] = None):
        ...
        logger.warning(f"🔒 MODULE EN QUARANTINE: {module_name} - Raison: {reason.value} - {duration}min")
    ...
    def _cleanup_expired_quarantines(self):
        ...
        expired = [
            module for module, quarantine in self.quarantined_modules.items() 
            if quarantine.is_expired
        ]
        ...
        if module_name in self.quarantined_modules:
            ...
            self.event_store.add_event(
                EventType.STATE_CHANGE,
                {
                    ...
                    "module": module_name,
                    "quarantine_duration_minutes": (datetime.now() - quarantine.quarantined_at).total_seconds() / 60
                },
                module="cognitive_reactor"
            )
    ...
    async def _check_module_health(self, context: Dict) -> List[CognitiveReaction]:
        ...
        for module in ["zeroia", "reflexia", "sandozia"]:
            ...
            if confidence < self.config["confidence_threshold"] and not self.is_module_quarantined(module):
...
def create_cognitive_reactor(behavior_analyzer: Optional[BehaviorAnalyzer] = None) -> CognitiveReactor:
