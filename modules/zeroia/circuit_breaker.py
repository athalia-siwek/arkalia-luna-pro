...
class CircuitBreaker:
    ...
    def _on_failure(self, exception: Exception) -> None:
        ...
        logger.warning(f"🚨 CircuitBreaker échec: {exception} (consécutif: {self.metrics.consecutive_failures})")
    ...
    def _log_blocked_call(self) -> None:
        ...
        self.event_store.add_event(
            EventType.CALL_BLOCKED,
            {
                "state": self.state.value,
                "consecutive_failures": self.metrics.consecutive_failures,
                "time_since_failure": (datetime.now() - self.last_failure_time).total_seconds() if self.last_failure_time else None
            }
        )
    ...
    def reset(self) -> None:
        ...
        logger.info(f"🔄 CircuitBreaker réinitialisé manuellement: {old_state.value} → {self.state.value}")
