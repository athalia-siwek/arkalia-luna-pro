...
try:
    from modules.sandozia.core.cognitive_reactor import CognitiveReactor, ReactionSeverity, QuarantineReason
    ...
    async def run_stress_test(self, cycles: int = 20, force_patterns: bool = True):
        ...
        for i in range(cycles):
            ...
            if result["cognitive_reactions"]:
                logger.warning(f"🔥 Cycle {i+1}: Réactions déclenchées: {result['cognitive_reactions']}")
        ...
        logger.info(f"🎯 Test de stress terminé:")
...
async def main():
    ...
    parser.add_argument("--integrate-with-zeroia", action="store_true", help="Test intégration ZeroIA")
    parser.add_argument("--generate-heatmap-data", action="store_true", help="Génère données heatmap")
