...
def test_event_store_initialization(temp_event_store):
    ...
    # cache_dir est un string, convertir en Path pour .exists()
    from pathlib import Path

    assert Path(temp_event_store.cache_dir).exists()
...
def test_type_index_functionality(temp_event_store):
    ...
    for i in range(3):
        temp_event_store.add_event(
            EventType.CIRCUIT_SUCCESS, {"success": f"success_{i}"}
        )
...
def test_event_store_persistence(temp_event_store):
    ...
    # Ajouter des événements
    event_id = temp_event_store.add_event(
        EventType.DECISION_MADE, {"decision": "monitor", "confidence": 0.9}
    )
    ...
    new_event_store = EventStore(
        cache_dir=str(temp_event_store.cache_dir), size_limit=10_000_000
    )
